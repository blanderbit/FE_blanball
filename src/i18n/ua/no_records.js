export default {
  noEvents: {
    title: 'Наразі немає актуальних подій для відображення',
    description: 'Ваша подія може стати першою',
    button_text: 'Організувати подію!',
  },
  noEventPlayers: {
    title: 'Наразі, немає учасників для відображення',
    description_author:
      'Як тільки хтось із користувачів приєднається — ми сповістимо вас',
    button_text: 'Організувати подію!',
    description_user:
      'Покищо ніхто не долучився до участі у події, однак, ви можете стати першим!',
  },
  noEventFans: {
    title: 'Наразі, немає глядачів для відображення',
    description_author:
      'Як тільки хтось із глядачів приєднається — ми сповістимо вас',
    button_text: 'Організувати подію!',
    description_user:
      'Покищо ніхто не долучився до вболівання у події, однак, ви можете стати першим!',
  },
  noMyEvents: {
    title: 'До сьогодні ви ще не були організатором подій',
    description: 'Організуйте вашу першу подію',
    button_text: 'Створити подію',
  },
  noMyTopicalEvents: {
    title: 'Наразі немає актуальних подій для відображення',
    description: 'Ваша подія може стати першою',
    button_text: 'Організувати подію!',
  },
  noMyFinishedEvents: {
    title: 'Наразі немає завершених подій для відображення',
    description: 'Ваша подія може стати першою',
    button_text: 'Організувати подію!',
  },
  noUsers: {
    title: 'Для такого фiльтру користувачiв не знайдено',
    description:
      'Знаходьте людей поблизу, збирайтеся в команди та грайте разом',
  },
  noNotifications: {
    title: 'Немає повідомлень для відображення',
    description: 'Вам ще не надходили сповіщення від інших користувачів',
  },
  noPopularEvents: {
    title:
      'Тут відображатимуться події, що мають найбільшу кількість активних учасників',
  },
  noPublicProfilePlannedEvents: {
    title: '{fullName} ще не додав жодної події до запланованих',
  },
  noPublicProfileReviews: {
    title: '{fullName} ще не отримував оцінок та коментарів',
  },
};
