    ...
    updateVisibleItems(itemsChanged, checkPositionDiff = false) {
      ...
      if (!count) {
        ...
        if (itemSize === null) {
          ...
          visibleEndIndex = Math.floor(
            ((scroll.end - beforeSize) / itemSize) * gridItems
          );
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        if (!view) {
          ...
          if (!view.nr.used) {
            console.warn(
              "Expected existing view's used flag to be true, got " +
                view.nr.used
            );
          }
        }
    ...
    scrollToPosition(position) {
      ...
      if (this.pageMode) {
        ...
        const scrollerPosition =
          scroller[direction.start] - bounds[direction.start];
