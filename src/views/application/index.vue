...
<script setup>
    ...
    const getToastOptions = (notificationInstance, toastId) => {
        const close = notificationInstance.actions.find(item => item.type === MessageActionTypes.Close);
        notificationInstance.actions = notificationInstance.actions.filter(item => item.type !== MessageActionTypes.Close);
        return {
            componentOptions: {
            ...
            options: {
                ...
                rtl: false,
                toastClassName: [
                    notificationInstance.getPushNotificationTheme()
                ]
            }
        }
    };
    ...
    const createToastFromInstanceType = (notificationInstance) => {
        ...
        const toastDataOptions = getToastOptions(notificationInstance, createUniqueId());
