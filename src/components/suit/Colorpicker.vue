<template>
  ...
  <div onselectstart="return false">
    <div
      class="cbtn"
      @click=";(isShowPanel = !isShowPanel), createpopper()"
      ref="cbtnEl"
    >
    ...
    <div
      ...
      v-show="isShowPanel"
      ref="panelEl"
      @contextmenu.prevent="bindContext"
    >
      <!--<div class="activeMode-wrapper">-->
      <!--<div class="btn-activeMode solid" :class="activeMode === 'solid' ? '' : 'gray'"-->
      <!--@click.stop="changeMode('solid')"></div>-->
      <!--<div class="btn-activeMode linear" :class="activeMode === 'linear' ? '' : 'gray'"-->
      <!--@click.stop="changeMode('linear')"></div>-->
      <!--<div class="btn-activeMode radial" :class="activeMode === 'radial' ? '' : 'gray'"-->
      <!--@click.stop="changeMode('radial')"></div>-->
      <!--</div>-->
      <div
        class="grad-wrapper"
        :style="activeMode === 'solid' ? 'display:none' : ''"
      >
        <div
          class="grad-bar"
          ref="gradBarEl"
          :style="{ backgroundImage: gradPreviewColor }"
          @click.stop="addGradPicker()"
        >
          <div
            v-for="(item, index) in gradColors"
            :key="item.id"
            style="top: -1px"
            class="picker"
            :class="index == activeGradPickerIndex ? 'on' : ''"
            @click.stop="
              tryDetectDbClick(
                () => getGradPickerPos($event.target, index),
                () => deleteGradPicker(index)
              )
            "
            @mousedown.stop="bindDown($event.target, getGradPickerPos, index)"
          ></div>
        </div>
        <div
          class="flex-row"
          :style="activeMode === 'linear' ? '' : 'visibility:hidden'"
        >
          <div
            class="degree"
            ref="degreeEl"
            @click.stop="getDegreePickerPos"
          ...
          <div style="font-size: 10px; width: 36px; padding-left: 8px">
            {{ degree }}°
          </div>
        </div>
      </div>
      <div class="palette-wrapper">
        <div
          class="palette"
          ref="paletteEl"
          @click.stop="getPalettePickerPos"
      ...
      <div class="huebar-wrapper">
        <div
          ...
          :style="isDropperEnabled ? '' : 'opacity: 50%;cursor:default'"
        ></div>
        <div class="bar-wrapper">
          <div
            class="hue-bar"
            ref="hueBarEl"
            @click.stop="getHuePickerPos"
          ...
          <div
            class="opacity-bar"
            ref="opactiyBarEl"
            @click.stop="getOpacityPickerPos"
...
props.gradients.forEach((item, index) => {
  g.push({
    ...
    color: Utils.rgba2hsba(item.color),
  })
})
gradColors.value = g
if (g.length > 0) {
...
function createpopper() {
  if (popper) return
  popper = createPopper(cbtnEl.value, panelEl.value, {
    placement: 'auto-end',
  })
}
...
let timer
function tryDetectDbClick(callback, dbCallback) {
  numClicks.value++
...
function bindOutsideClick() {
  ...
  function isInObject(el) {
    ...
    if (
      mousePos.x > elPos.left + el.offsetWidth ||
      mousePos.x < elPos.left ||
      mousePos.y > elPos.top + el.offsetHeight ||
...
function deleteGradPicker(index) {
  gradColors.value.splice(index, 1)
}

function addGradPicker() {
  if (gradColors.value.length < 10) {
    const c = JSON.parse(
      JSON.stringify(gradColors.value[activeGradPickerIndex.value])
    )
    c.id = gradMaxId++
    ...
    nextTick(() =>
      getGradPickerPos(
        gradBarEl.value.children[activeGradPickerIndex.value],
        activeGradPickerIndex.value
      )
    )
  }
}
...
function setGradPickerPos() {
  gradColors.value.forEach((item, index) => {
    gradBarEl.value.children[index].style.left =
      ((barWidth - 9) / 100) * item.percent - 3 + 'px'
  })
}
...
function updatePreviews() {
  ...
  switch (activeMode.value) {
    case 'solid':
      ...
      break
    case 'linear':
      gradPreviewColor.value = `linear-gradient(to right,${gradStr.slice(
        1
      )}),url('${maskImgUrl}')`
    ...
    case 'radial':
      ...
      previewBackground.value = `radial-gradient(${gradStr.slice(
        1
      )}),url('${maskImgUrl}')`
...
function getDegreePickerPos() {
  ...
  const rad = Math.atan2(
    elPos.top + r - mousePos.y,
    mousePos.x - elPos.left - r
  )
...
function getPalettePickerPos() {
  ...
  const left = Math.max(
    -6,
    Math.min(mousePos.x - elPos.left - 6, paletteWidth - 6)
  )
  const top = Math.max(
    -6,
    Math.min(mousePos.y - elPos.top - 6, paletteHeight - 6)
  )
...
function getOpacityPickerPos() {
  ...
  const left = Math.max(
    -3,
    Math.min(barWidth - 12, mousePos.x - elPos.left - 6)
  )
...
function setPickerPos() {
  ...
  paletteEl.value.style.background = `rgb(${c.r},${c.g},${c.b},1)`
  palettePickerEl.value.style.left =
    (paletteWidth / 100) * paletteColor.s - 6 + 'px'
  palettePickerEl.value.style.top =
    paletteHeight - (paletteHeight / 100) * paletteColor.b - 6 + 'px'
  huePickerEl.value.style.left =
    ((barWidth - 9) / 360) * paletteColor.h - 3 + 'px'
