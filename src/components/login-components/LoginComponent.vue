    ...
    <div class="b-login-step">
        <Form v-slot="data" :validation-schema="schema">
            ...
            <div class="b-login-step__buttons">
                <GreenBtn @click-function="handleLogin(data)" :text="'Увійти в акаунт'" :height="40"/>
            </div>
        </Form>
    ...
    export default {
        ...
        setup() {
            ...
            const handleLogin = async (data) => {
                ...
                try {
                    const apiRequestResult = await API.AuthorizationService.login(data.values);
                    ...
                    if(redirectUrl) {
                        ...
                        if(resolveRouter?.matched?.find(match => match?.path?.includes('pathMatch'))) {
